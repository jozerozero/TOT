...
class NPTransitionPrior(nn.Module):
    ...
    def __init__(self, lags, latent_size, num_layers=3, hidden_dim=64, compress_dim=3):
        super().__init__()
        self.lags = lags
        self.latent_size = latent_size
...
class net(nn.Module):
    ...
    def forward_individual(self, x, x_mark,is_training):
        # print('x', x.shape)
        ...
        x = torch.cat([x_temp, zs_rec], dim=-1).transpose(1, 2)
    ...
    def forward_weight(self, x, x_mark, g1, g2,is_training):
        ...
        zs_ = self.encoder1.encoder.forward_time(x_temp)
        zs_ = self.zs_rec(zs_)
        zs_rec_mean, zs_rec_std = zs_[:, :, :self.args.seq_len], self.sigmoid(zs_[:, :, -self.args.seq_len:])
        if is_training:
            zs_rec = self.__reparametrize(zs_rec_mean, zs_rec_std)
        ...
        if is_training:
            ...
            obs_kl_loss = self.obs_kl_loss(x_temp, zs_rec_mean, zs_rec_std, zs_rec)
            other_loss = zs_kl_loss * self.args.zc_kl_weight + obs_kl_loss * self.args.obs_weight
        else:
            other_loss = 0

        zs_rec_transposed = zs_rec if zs_rec.size(1) == rep.size(1) else zs_rec.permute(0, 2, 1)
        rep = torch.cat([rep,zs_rec_transposed], dim=-1)
...
class Exp_TS2VecSupervised(Exp_Basic):
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
                ...
                loss = criterion(pred[0], true)+ criterion(pred[1], true)+ other_loss
    ...
    def _process_one_batch(self, dataset_object, batch_x, batch_y, batch_x_mark, batch_y_mark, mode='train'):
        ...
        if mode == 'train' :
            is_training = True
        elif mode =='test' and self.online != 'none':
            is_training = True
        else:
            is_training = False
