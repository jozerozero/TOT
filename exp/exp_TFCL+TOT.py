...
class net(nn.Module):
    def __init__(self, args, device):
        ...
        encoder = TSEncoder(input_dims=2*args.enc_in + 7,
                             ...
                             hidden_dims=128, # standard ts2vec backbone value
                             depth=10) 
...
class Exp_TS2VecSupervised(Exp_Basic):
    ...
    def test(self, setting):
        ...
        # preds_np = [tensor.numpy() for tensor in preds]
        # trues_np = [tensor.numpy() for tensor in trues]
        # x_list_np = [tensor.numpy() for tensor in x_list]
        # np.save(f'./draw_data/{self.args.method}/{self.args.data}/{self.args.pred_len}/pred.npy', np.array(preds_np))
        # np.save(f'./draw_data/{self.args.method}/{self.args.data}/{self.args.pred_len}/y.npy', np.array(trues_np))
        # np.save(f'./draw_data/{self.args.method}/{self.args.data}/{self.args.pred_len}/x.npy', np.array(x_list_np))
