...
from .nsts_ import DilatedConvEncoder
...
class TSEncoder(nn.Module):
    def __init__(self, args, input_dims, output_dims, hidden_dims=64, depth=10, mask_mode='binomial', gamma=0.9):
        super().__init__()
    ...
    def forward_time(self, x, mask=None):  # x: B x T x input_dims
        ...
        x[~nan_mask] = 0
        # x = torch.float(x)
        x = self.input_fc(x.float())  # B x T x Ch
